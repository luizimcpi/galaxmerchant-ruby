...
## Solution
```
Basically the problem was solved using a reader class (read_files.rb) that is responsible to read the file and show the output. 
There is another class responsible to show (output_generator.rb) the correct output but this class is not responsible to format the 
output, for that there is a formatation class (output_formatter.rb) and finally there is a class to convert the coin sentences into number values (output_calculator.rb). The number class helps the calculator with conversions and Checker class (checker_words.rb) contains rules to verify that sentences are valid for each line of the file.
```